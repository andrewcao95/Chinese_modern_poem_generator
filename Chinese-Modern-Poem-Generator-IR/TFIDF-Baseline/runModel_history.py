        ...
        for ii, keyword in enumerate(test_sentences[i].split(' ')):
            ...
            if ii == 0:
                ...
                for ii, ans in enumerate(q_reverse):
                    _, sims_last_sentence = ss.similarity(ans)
                    sims = [sims_curr[i] + sims_last_sentence[i-1] * math.exp(-(ii + 1)) if i > 0 else sims_curr[i] for i in range(train_sentences_len) ]
